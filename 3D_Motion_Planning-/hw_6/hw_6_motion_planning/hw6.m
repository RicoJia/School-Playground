...
n_order = 7;   % 8 control points, minisnap
n_seg = size(path, 1) - 1;
...
for k = 1:n_seg
    for t = 0:0.01:1
        ...
        for i = 0:n_order
            ...
            x_pos(idx) = x_pos(idx) + basis_p * poly_coef_x((k-1) * (n_order + 1) + i +1, 1);
            y_pos(idx) = y_pos(idx) + basis_p * poly_coef_y((k-1) * (n_order + 1) + i + 1, 1);
        end
        idx = idx + 1;
    end
    plot(x_pos, y_pos)
...
function poly_coef = MinimumSnapCorridorBezierSolver(axis, waypoints, corridor, ts, n_seg, n_order, v_max, a_max)
    ...
    sz = size(corridor);
    for cln_idx = 1:sz(2)
        % lower and upper limits of one axis in one column, (2, n_seg-1)
        corridor_range(:, cln_idx) = [corridor(axis, cln_idx) - corridor(axis+2, cln_idx),  corridor(axis, cln_idx) + corridor(axis+2, cln_idx)]';
    end

...
class Node:
    ...
    def generate_key(px, py, vx, vy):
        """
        generate unique string of position and vel
        """
    ...
    def connect_to_graph(self, grid):
        """
        For each possible action, store a string that represents its velocity, position into its list
        """
    ...
    def velocity_constraints(vx, vy):
        """return velocities within the constraints"""
    ...
    def control(self, ux, uy, grid, success):
        """
        Get vx, vy, and update their positions accordingly. Then generate a unique key
        """
        ...
        vx = self.vx + ux
        vy = self.vy + uy
        vx, vy = self.velocity_constraints(vx, vy)  # velocities within boundaries
        px = self.px + vx   # next state
        py = self.py + vy

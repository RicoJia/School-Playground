...
graph = {} #{(position, vel): state_node}



def build_up_graph(grid, save_path):
    """
    each grid cell has m velocities as states. 
    original g value is dist(state, finish)   TODO:  what dist? 
    calculate the cell's state = (pos, velocity), and find neighboring states
    """
    max_vel = 5
    ...
    while bellman_error > 0.0001:
        ...
        for key in graph.keys():
            ...
            if state.is_goal:
                ...
                value_uk = []
                # for each action, calculate its expected value
                for child_idx in range(len(ACTION_SPACE)):
                    child_key_9 = state.next_prob_9[child_idx]
                    child_9 = graph[child_key_9]    #node for next state p = 0.9
...
if __name__ == '__main__':
    ...
    # build_up_graph(track_map, path)     #note: not optimal yet

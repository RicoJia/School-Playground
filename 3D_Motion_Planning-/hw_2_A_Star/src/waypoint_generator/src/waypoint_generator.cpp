...
// transform waypoints on segment into valid robot poses, then push into waypoints
void load_seg(ros::NodeHandle& nh, int segid, const ros::Time& time_base) {
    ...
    ROS_ASSERT(nh.getParam(seg_str + "x", ptx));    //load segidx from param server. All xs for points? 
    ...
    double baseyaw = tf::getYaw(odom.pose.pose.orientation);  //base yaw from odom

...
{   
    ...
    // Store ROS point cloud into a map
    pcl::fromROSMsg(pointcloud_map, cloud);
    
    if( (int)cloud.points.size() == 0 ) return;

    // adding each point to RRT star
    pcl::PointXYZ pt;
...
// Our own collision checker. For this demo, our robot's state space
// From ompl
class ValidityChecker : public ob::StateValidityChecker
{
public:
    // we store [x,y,z] in state
...
// RICO: example https://ompl.kavrakilab.org/OptimalPlanning_8cpp_source.html
void pathFinding(const Vector3d start_pt, const Vector3d target_pt)
{
    // Construct the robot state space in which we're planning. 
    ob::StateSpacePtr space(new ob::RealVectorStateSpace(3));

    // Set the bounds of space to be in [0,1]. (unsigned int index, double value)
    ...
    // STEP 5: Set the optimization objective, the options you can choose are defined earlier: getPathLengthObjective() and getThresholdPathLengthObj()
    // ? 
     pdef->setOptimizationObjective(getPathLengthObjective(si));


     //STEP 6: Construct our optimizing planner using the RRTstar algorithm, please define varible as optimizingPlanner
    ...
    if (solved)
    {
        ...
        for (size_t path_idx = 0; path_idx < path->getStateCount (); path_idx++)
        {
            ...
            path_points.emplace_back(Vector3d{state->values[0], state->values[1], state->values[2]});
        }

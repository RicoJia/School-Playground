...
public:
  ...
  : Node("many_topics_listener")
  {
    ...
    if (message_size_ == "128B") {
      subscriptions_128b_.reserve(num_topics_);
      for (int i = 0; i < num_topics_; ++i) {
        int topic_id = node_num_ * num_topics_ + i;
        std::string topic_name = "test_topic_" + std::to_string(topic_id);
        auto sub = create_subscription<shm_tests::msg::FixedData128B>(
          topic_name, qos,
          [this](const shm_tests::msg::FixedData128B::SharedPtr msg) {
        ...
        subscriptions_128b_.push_back(sub);
      }
...
private:
  ...
  std::vector<rclcpp::Subscription<shm_tests::msg::FixedData128B>::SharedPtr> subscriptions_128b_;

...
#include "shm_tests/msg/fixed_data128_b.hpp"
...
public:
  ManyTopicsTalker(int node_num) : Node("many_topics_talker"), count_(0), node_num_(node_num)
  {
    ...
    if (message_size_ == "128B") {
      // Create 128B publishers
      for (int i = 0; i < num_topics_; i++) {
        int topic_id = node_num_ * num_topics_ + i;
        std::string topic_name = "test_topic_" + std::to_string(topic_id);
        auto pub = this->create_publisher<shm_tests::msg::FixedData128B>(topic_name, qos);
        publishers_128b_.push_back(pub);
      }
    } else if (message_size_ == "1KB") {
    ...
    size_t pub_count = message_size_ == "128B" ? publishers_128b_.size() : 
                        (message_size_ == "1KB" ? publishers_1kb_.size() : publishers_15kb_.size());
...
private:
  void timer_callback() {
    ...
    if (message_size_ == "128B") {
      publish_128b(total_publish_duration_ns, publish_count);
    } else if (message_size_ == "1KB") {
  ...
  void publish_128b(int64_t& total_duration_ns, size_t& count) {
    if (zero_copy_) {
      // Zero-copy mode: Use loan API
      for (size_t i = 0; i < publishers_128b_.size(); i++) {
        try {
          auto publish_start_ns = this->get_clock()->now().nanoseconds();
          
          // Borrow a loaned message from shared memory
          auto loaned_msg = publishers_128b_[i]->borrow_loaned_message();
          ...
          // Fill the payload (128 - 12 = 116 bytes)
          for (size_t j = 12; j < 128; j++) {
            data[j] = static_cast<uint8_t>(j % 256);
          }
          
          // Publish the loaned message (no copy, just transfers ownership)
          publishers_128b_[i]->publish(std::move(loaned_msg));
      ...
      for (size_t i = 0; i < publishers_128b_.size(); i++) {
        ...
        auto message = shm_tests::msg::FixedData128B();
        ...
        publishers_128b_[i]->publish(message);
  ...
  std::vector<rclcpp::Publisher<shm_tests::msg::FixedData128B>::SharedPtr> publishers_128b_;
